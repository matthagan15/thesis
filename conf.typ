...
#let ut-thesis(
    ...
    page[
        #align(center)[
            ...
            #text(size: 14pt, weight: "bold", font: "New Computer Modern")[Abstract]
        ]
        #set par(leading: 1.5em, justify: true)
        #set text(font: "New Computer Modern")
        The simulation of quantum systems is the most promising candidate for computational advantages of large scale, fault-tolerant quantum computers over classical computers. Two fundamental problems that appear when simulating quantum systems on quantum computers are the preparation of good input states to the simulation and the problem of decomposing the time evolution operator $e^(i H t)$ into primitive gates that are available on the quantum computer. This thesis presents two new algorithms for solving these problems. The first is a time evolution algorithm that approximates the time evolution operator by combining two previously developed techniques. We bound the error of this composite approach and determine analytic conditions on when this can lead to cost savings of the overall simulation. These savings are then verified through extensive numerics. This simulation algorithm can then be used as a subroutine to prepare thermal states, which are typical input states that have a high overlap with the ground state. Our algorithm is inspired by Hamiltonian Monte Carlo, which is a classical sampling algorithm that utilizes Hamiltonian dynamics to sample from arbitrary probability distributions. We bound the cost of our quantum thermal state preparation algorithm in terms of the total amount of time needed to simulate the Hamiltonian of the system in four scenarios: single qubit systems, truncated harmonic oscillators, non-degenerate systems where one knows the eigenvalues, and non-degenerate systems where no eigenvalue knowledge is present. We numerically demonstrate that this algorithm works as guaranteed and can significantly outperform analytic worst-case bounds.
    ]
