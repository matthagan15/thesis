...
To resolve this apparent discrepancy, physicists have settled on three main approaches: the Eigenstate Thermalization Hypothesis (ETH), Liouvillian based evolutions, and the Repeated Interactions (RI) framework. ETH studies thermalization without giving up the notion of a closed quantum system. In this framework the system is taken to be large, as in thermodynamically large, and thermalization is observed whenever the statistics of a _local_ observable that acts only on a small, constant number of particles. In chaotic systems these observables can be rigorously shown to appear as if they came from the thermal average $e^(-beta H_"local") / cal(Z)_"local"$, but showing that similar techniques work for non-chaotic systems is a major open question. In fact, the existance of many-body localization serves as a counterexample to a universal ETH, but the existance of these phases of matter in the real world, along with the validity of ETH in general, are heavily debated.

The second approach based on Liouvillian schemes abandons the notion of a closed quantum system and instead studies the effect of an environment on the system. This requires abandoning unitary evolution and working with Liouvillian maps, which are a particular way to express the effects of a quantum channel on the system-environment Hilbert space as a mapping only on the system Hilbert space. This means that even the effects of an infinitely large environment can in principle be simulated on finite sized quantum devices. These techniques were used by Davies @davies1974markovian to show how an infinite dimensional environment with weak coupling to the system can lead to thermal systems. Extending these techniques to a greater variety of system-environment scenarios, such as the strong coupling regime @anto2023effective or non-Markovian environments @anto2021capturing, is a key component of open quantum systems research.

The last approach is the Repeated Interactions (RI) framework, which sits somewhere between the completely closed system approach of ETH and the infinitely large environments that are typically assumed in open quantum systems. The RI prescription does not abandon the notion of closed quantum systems, but instead opts to directly simulate a very small environment, oftentimes a single extra qubit or spin. The most straightforward example is to imagine a single photon $gamma$ drawn from some black-body thermal spectrum. This single photon then interacts with the system of interest for a brief period, only to then fly off and never interact with the system again. We then can take a new photon from this thermal background and interact with the system again, but with a refreshed photon. The idea is that by repeating this over and over, if the interaction is properly chosen, then many photons can collectively thermalize the system. Two drawbacks with the RI approach is that a specific model for the environment must be chosen and a specific interaction between the system and environment must be determined. For discrete systems this has typically restricted RI results to systems that can be handled analytically, including two-level systems @prositto2025equilibrium, the XX chain model @karevski2009quantum, and wave packet interactions for continuous single particle systems @jacob2021thermalization.

One of the main contributions of this thesis is demonstrating that the problem of constructing interactions between a system and environment can be eliminated through the use of random matrices. By choosing the eigenvalues and eigenvectors of an interaction $G$ through sufficient random ensembles, we show that thermal states for any finite dimensional system can be prepared in finite time. We show how to analyze this channel in a weak-coupling regime, which is effectively a Taylor Series with respect to a coupling $alpha$. This expansion then reveals a Markov chain underlying the resulting channel on the system. By using basic tools from Markov Chain analysis we can then compute the fixed points of the map and bound the number of interactions needed to converge to the fixed point. We then give new conditions that are analogous to the detailed-balance conditions used in Markov Chain analysis that can be used to check if the thermal state is the fixed point of the thermalizing channel. The beauty of our technique is that it works for any non-degenerate system with or without knowledge of the eigenvalues of the system, although without eigenvalue knowledge we are only able to show that the thermal state is an approximate fixed point for finite $beta$ but exactly fixed in the limit $beta -> oo$.

The rest of this chapter is organized as follows. In @sec_tsp_intro we briefly discuss related quantum algorithms and provide a summary of the main technical results. In @sec_tsp_weak_coupling we develop the weak-coupling expansion and provide necessary analysis about the underlying Markov chain. In @sec_tsp_oscillator we then take these results and show how to use them to prepare thermal states for single qubit systems and for truncated harmonic oscillators. This section can be viewed as a warmup to the more general results contained in @sec_tsp_generic_sys, but we present the results separately as they utilize slightly different techniques and are more readily comparable to existing approaches. In @sec_tsp_generic_sys we study generic systems from two perspectives, one in which no eigenvalue knowledge is present and the other in which eigenvalues are known. Finally we include a discussion on interpretations of these results and possible extensions in @sec_tsp_discussion.
...
The remainder of this chapter is split into three main parts. @sec_tsp_weak_coupling contains a derivation of the weak-coupling expansion in @thm_tsp_second_order_expansion and outlines the underlying Markov chain behavior in @sec_tsp_markovian_dynamics. This weak-coupling expansion is presented in as much generality as possible as it may be of use in other applications beyond our thermalization procedure, such as thermometry or spectroscopy. @sec_tsp_oscillator has two theorems concerning single qubit systems and harmonic oscillators, @thm_single_qubit and @thm_harmonic_oscillator respectively, as well as numerics exploring the $beta$ and $epsilon$ dependence of the channel in @sec_specific_numerics. @sec_tsp_generic_sys contains our most general results, where we prove thermalization bounds for non-degenerate systems with no eigenvalue knowledge in @thm_zero_knowledge and with perfect eigenvalue knowledge in @thm_perfect_knowledge.

One of the key aspects of our thermalization procedure is that the analysis is dependent on the ability to tune the environment energy gap, denoted $gamma$, to match the system energy differences. One of our main results in @thm_zero_knowledge shows that even if the user cannot tune $gamma$ at all and is reduced to uniform guessing within an interval containing all of the system's energy differences, denoted $Delta_S (i,j)$, then thermalization can still occur. We show that the thermal state at finite $beta$ is an approximate fixed state, with the error going to 0 as the coupling constant $alpha -> 0$. This zero coupling limit can be taken with the opposite limit $t -> infinity$ to yield a nonzero simulation time for the random interaction $G$. Further, we show that the ground state is exactly the fixed point in the $beta -> infinity$ limit. In this limit we are also able to bound the total simulation time required as $L dot t in tilde(O)(frac(dim_S^(16) norm(H_S)^7, delta_("min")^8 epsilon^6))$, where $delta_("min")$ represents a "resolution" type distance and is the smallest difference between two distinct eigenvalue differences $|Delta_S (i,j) - Delta_S (k,l)|$. When preparing finite $beta$ thermal states we pick up an extra factor of $frac(1, tilde(lambda)_star (beta)^7)$ related to the spectral gap of the transition matrix.
...
$
    G = U_(G) Lambda_G U_(G)^dagger, " where " U_(G) tilde "Clifford"(hilb_S tp hilb_E) text(", ") Lambda_G = (-1)^(z_0) Z_1^(z_1) tp ... tp Z_n^(z_n),
$ <eq_interaction_def>
and the coefficients $z_i$ are sampled I.I.D via the distribution $"Pr"[z_i = 0] = "Pr"[z_i = 1] = 1 / 2$. The eigenvectors are chosen to be a uniformly random Clifford circuit, which we choose due to the fact that the Cliffords form a 2-design @webb2015clifford. We then add this random interaction term to our system-environment dynamics with a coupling constant $alpha$, yielding a total dynamics governed by $H_S + H_E + alpha G$. We define the following rescaled coupling constant
$
    tilde(alpha) := frac(alpha t, sqrt(dim + 1)),
$ <eq_a_tilde_def>
...
One of the advantages of the channel given in @eq:PhiDef is that it is very easy to implement on a quantum computer. For a fixed value of $gamma$ and $beta$ we can prepare the thermal state $rho_E (beta; gamma)$ by starting the qubit in the $ketbra(0, 0)$ state. Then with probability $q1$ flip the qubit by applying an $X$ gate, giving the mixed state $q0 ketbra(0, 0) + q1 ketbra(1, 1) = rho_E (beta; gamma)$. We then sample an interaction term by sampling $n + 1$ uniformly random bits to obtain the eigenvalues $Lambda_G$. Then we sample a uniform Clifford, which can be done straightforwardly using the algorithm by van den Berg @van2021clifford in time $O(n^2)$. We then can use this Clifford circuit to conjugate our randomly chosen Pauli $Z$ string to a different Pauli operator using a stabilizer Tableau formalism, which can be done in polynomial time in $n$ using software such as Stim @Gidney2021stimfaststabilizer. This Pauli operator $G$ is then appended to the Hamiltonian $H = H_S + H_E$ and any Hamiltonian simulation algorithm can be used to implement $e^(i (H + alpha G)t)$. Implementing the partial trace is hardware dependent, so we leave that up to the user. We further do not make an attempt to bound the number of samples needed but numerics indicate a rapid convergence to the mean.
...
#proof()[
    ...
    $
        frac(partial, partial alpha) Phi (rho) |_(alpha = 0) &= frac(partial, partial alpha) tr_(cal(H)_E) EE_G Phi_G (rho) |_(alpha = 0) \
        &= tr_(cal(H)_E)(EE_G frac(partial, partial alpha) Phi_G(rho) |_(alpha = 0)) .
    $
...
#lemma()[
    ...
    $
    and let $eta(i, j)$ denote the degeneracy of the $(i,j)^"th"$ eigenvalue of $H = H_S + H_E$. Then the $O(alpha^2)$ term of $EE_G Phi_G$ in a weak-coupling expansion is given by
    $
...
#proof([of @thm_tsp_second_order_expansion])[
    ...
    Since we have shown that coherences are not introduced to our system we can focus on the transitions from diagonal entries to diagonal entries in $rho$. We make heavy use of @eq_el_gigante_dos which tells us that for $i != k$ the system-environment transition amplitude is
    $
...
#proof()[
    ...
    $
        norm(cal(T)_"off")_1 + norm(R_Phi)_1 <= (8 alpha^2) / (Delta^2) + 4 dim_S (alpha t)^3.
    $
    by setting $alpha = 1\/(dim_S Delta^2 t^3)$ we can simplify the above as
    $
        norm(cal(T)_"off")_1 + norm(R_Phi)_1 <= (alpha^2) / (Delta^2) 12.
    $
...
#figure(
    grid(
        columns: 2,
        row-gutter: 3mm,
        image("tsp_numerics/sho_total_time_vs_beta_dim_4.svg"), [#v(6mm) #image("tsp_numerics/spec_gap_dim_4.svg")],
    ...
    caption: [
        Demonstration of $beta$ dependence of the thermalizing channel $Phi$ for the truncated harmonic oscillator. The environment gap $gamma$ was tuned to match the system gap $Delta$ exactly. The minimal number of interactions was found by binary search over values of $L$ that have an average error of less than $epsilon = 0.05$ with 100 samples.],
...
We now extend our thermalization techniques to arbitrary Hamiltonians with no degenerate eigenvalues. The first major difficulty that we run into is how to choose our environment gap $gamma$. If one does not have any knowledge whatsoever about where the eigenvalues of $H_S$ may lie then we are reduced to uniform guessing. In @sec_tsp_zero_knowledge we show that even in this scenario the thermal state is an approximate fixed point for finite $beta$ and the exact fixed point for ground states and we provide a bound on the total simulation time required. However, we show that this generality does come at a cost. If one has complete knowledge of the eigenvalue differences we show in @sec_tsp_perfect_knowledge that the total simulation time markedly decreases. Further, with complete knowledge the thermal state is an exact fixed point for all $beta$. Finally, in @sec_tsp_generic_numerics we study these impacts on small Hydrogen chain systems and observe the quantitative effects of noise added to $gamma$.
...
#theorem([Zero Knowledge Thermal State Prep])[
    ...
    + For finite $beta$ the thermal state is an approximate fixed point of the thermalizing channel $EE_gamma Phi_gamma$ with a deviation of $ norm(rho_S (beta) - EE_gamma Phi_gamma (rho_S (beta)))_1 <= alpha^2 t e^(beta delta_"min") norm(H_S)^(-1) pi + 8 (alpha^2) / (delta_"min") + 4 "dim"_S (alpha t)^3. $
...
#proof()[
    ...
    $
        norm(cal(T)_"off")_1 + norm(R_Phi)_1 <= (8 alpha^2) / (delta_min^2) + 4 dim_S (alpha t)^3 = alpha^2 / delta_min^2 12,
    $
...
#proof()[
    ...
    $
        norm(cal(T)_"off")_1 + norm(R_(Phi))_1 <= (8alpha^2) / (delta_"min"^2) + 4 "dim"_S (alpha t)^3 = (alpha^2) / (delta_"min"^2) 12 alpha "dim"_S delta_"min"^2 t^3).
    $
...
One benefit of our thermalization procedure is that it can be compiled all the way down to the circuit level with minimal overhead in complexity. The thermal state of the ancilla can be prepared by flipping a clean ancilla qubit with probability $q1$ and a random Pauli can be chosen via the process given in @sec_tsp_prelims. This random Pauli is then added to the simulation procedure of the user's choice to implement $e^(i (H_S + H_E + alpha G) t)$. We note that this uses uncontrolled time evolution, as opposed to many Linbladian based approaches which use controlled time evolution to implement a Fourier weighted jump operator.
