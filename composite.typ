...
One of the main drawbacks of Trotter-Suzuki formulas is that each term in the Hamiltonian has to be included in the product formula, regardless of the magnitude of the term. This leads to a circuit with a depth that scales at least linearly with the number of terms in $H$, typically denoted $L$. QDrift avoids this by randomly choosing which term to implement next in the product formula according to an importance sampling scheme in which higher weight terms have larger probabilities. The downside to QDrift is that it has the same asymptotic scaling with $t / epsilon$ as a first-order Trotter formula, meaning it is outperformed at large $t / epsilon$ by even a second-order Trotter formula.

The main results of this chapter are focused on the idea of partitioning a Hamiltonian up into two parts and simulating one part with QDrift and the other with Trotter. We provide upper bounds on the error, and therefore the gate cost for a simulation, associated with a composite channel for a given partitioning using first-order Trotter formulas in @thm_composite_first_order_cost and higher-order formulas in @thm_composite_higher_order_cost. We then give analytic conditions on when the simulation cost of the composite approach can outperform either of its constituent methods in @thm_composite_higher_order_improvements and provide a partitioning for Hamiltonians that have an exponential decay in the spectral norm of each term in @thm_composite_probabilistic_improvements. Lastly, we provide significant numeric evidence in @sec_composite_numerics that these techniques can lead to improvements for chemical benchmark systems.

== Related Work
The development of these composite techniques was motivated from two ideas, one was to extend randomized compilers to include conditional probabilities and the other was to formalize the process in chemistry simulations of dropping small weight terms or shuffling terms around different time steps @bucket_sim. We were unable to completely formalize the notion of conditional samples for a QDrift randomized compiler, but these techniques can be seen as a specific distribution in which a certain subset of terms can only be sampled once all of the deterministic terms have been processed. The latter concept of shuffling terms was first developed with the idea of "coalescing" terms into "buckets" by Wecker et al. @bucket_sim and further explored by Poulin et al. @coalescing_con_wiebe. They showed that grouping terms of similar sizes together to be skipped during certain Trotter steps led to negligible increases in error and reduced gate counts by about a factor of 10. Similar improvements are also seen in the randomized setting of @kivlichan2019phase.

More recently, simulation approaches have sought to use the advantages of randomized compilation as a subset of an overall simulation, such as the hybridized scheme for interaction picture simulations @hybridized_interaction_pic. What separates these two works is that our approach offers a more flexible approach for generic time-independent simulation problems whereas the hybridized schemes are specifically tailored to taking advantage of the time dependence introduced by moving to an interaction picture. As such, the hybridized approach achieves asymptotic advantages when the size of the interaction picture term dominates the overall Hamiltonian. This typically occurs in instances in which the size of an operator is unbounded, which can occur in lattice field theory simulations or constrained systems. The way the hybridized scheme in @hybridized_interaction_pic works is via a "vertical" stacking of simulation channels, for example one channel to handle the Interaction Picture rotations and then other channels on top of this to simulate the time-dependence it generates on the remaining Hamiltonian terms. Our work instead remains in the Schrodinger time evolution picture and we perform a "horizontal" stacking of simulation techniques. By horizontal we mean for a given simulation time we split the Hamiltonian up into (potentially) disjoint partitions and simulate each partition for the full simulation time but with different techniques, such as Trotter or QDrift. These techniques allow us to achieve asymptotic improvements over either method for a loose set of assumptions.
...
#figure(
    grid(
        columns: 2,
        row-gutter: 5mm,
        image("composite_numerics/H3_nb_new_legend.png"), image("composite_numerics/H3_w_new_legend.png"),
        "(a)", "(b)",
    ),
...
#figure(
    grid(
        columns: 2,
        row-gutter: 3mm,
        image("composite_numerics/graph7_new_legend.png"), image("composite_numerics/graph8_new_legend.png"),
